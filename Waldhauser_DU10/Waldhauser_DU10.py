...
Posilované učení v nebezpečném světě
    ...
    Naimplementujte způsob, jak procházet světem a vhodně kombinovat exploraci a exploataci, 
    abyste se dozvěděli strategii, kterou pro každé políčko hry máte ideálně využít.
   
    Prostředí máte předimplementované v příslušném souboru k příslušné kapitole
    
    Při přechodu akcí ze stavu obdržíte pomocí funkce move(x, y, action) nový stav,
    odměnu a info, zda je stav terminální, agent ale nesmí znalosti této funkce využívat a dozvídá se informaci o odměně a novém stavu vždy po jednom kroku
    Jak jsou implementované akce a prostředí zjistíte v příslušném souboru (pokud to budete potřebovat, dejte pozor na reprezentaci scény v poli prostředí), 
    ve světě jsou souřadnice x, y brány tak jak jsme zvyklí v kartézské soustavě souřadnic
"""
